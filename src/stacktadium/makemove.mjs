...
const TOMB_SQUARE_OFFSETS = [-11, -10, -9, -1, 0, 1, 9, 10, 11];
...
export function MakeMove(move, moveType = '') {
  ...
  if (TOSQ(move) > 0 && isSummon(move)) {
    if (captured === 6) {
      ...
      for (let i = 0; i < TOMB_SQUARE_OFFSETS.length; i++) {
        const sq = to + TOMB_SQUARE_OFFSETS[i];
        ...
        if (GameBoard.pieces[sq] === PIECES.wEXILE) continue;
        if (GameBoard.pieces[sq] === PIECES.bEXILE) continue;
